...
import numpy as np
from mip import BINARY, CONTINUOUS, Model, maximize, xsum
...
class PythonMipModel(SolverBase):
    """Python MIPによる番組選択最適化クラス"""
    ...
    def _add_constraints(self):
        ...
        # 補助変数の漸化式制約
        for i in self.input_data.list_indivisual_index:
            self.model += self.e[i, 0] == 1

            for k in self.input_data.list_program_index:
                self.model += -self.d[k] <= self.e[i, k] - self.e[i, k - 1]
                self.model += self.e[i, k] - self.e[i, k - 1] <= self.d[k]

                self.model += -(1 - self.d[k]) <= self.e[i, k] - self.e[i, k - 1] * (
                    1 - self.input_data.individual_reach_probs[i - 1, k - 1]
                )
                self.model += self.e[i, k] - self.e[i, k - 1] * (
                    1 - self.input_data.individual_reach_probs[i - 1, k - 1]
                ) <= (1 - self.d[k])
    ...
    def solve(self) -> OptimizeOutputData:
        ...
        if status.name in ["OPTIMAL", "FEASIBLE"]:
            # 選択された番組を取得
            selected_programs = []
            for j in self.input_data.list_program_index:
        ...
        return OptimizeOutputData(
            ...
            objective_value=objective_value,  # type: ignore
